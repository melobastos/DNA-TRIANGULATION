...
if st.session_state["comparisons"]:
    ...
    y_gap = 1.0  # Espaço entre as faixas
    ...
    for chrom in unique_chromosomes:  # Apenas cromossomos com segmentos
        ...
        for i, (_, row) in enumerate(chrom_data.iterrows()):
            color = color_map[row["Comparison"]]
            ax.add_patch(plt.Rectangle((row["Start"], y_offset - (i * y_gap)), 
                                       row["End"] - row["Start"], 0.8, color=color, alpha=0.8))
